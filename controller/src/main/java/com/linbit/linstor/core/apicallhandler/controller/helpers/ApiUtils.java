...
public class ApiUtils
{
    ...
    public static ResourceName createResourceName(final byte[] genInput)
        throws InvalidNameException
    {
        ...
        for (Mode genMode = Mode.PREFIX_UUID; rscName == null && genMode != Mode.FAIL; genMode = genMode.advance())
        {
            switch (genMode)
            {
                case PREFIX_UUID:
                    ...
                    try
                    {
                        String uuidStr = API_UUID_NAME_PREFIX + UUID.fromString(truncName).toString().toUpperCase();
        ...
        if (rscName == null)
        {
            ...
            //throw new InvalidNameException(
            //    "Generation of a " + LinStor.PROGRAM + " resource name from name generator input data failed",
            //    new String(genInput)
            //);
        }
