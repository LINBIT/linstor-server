...
public final class StdErrorReporter implements ErrorReporter
{
    ...
    // The drbdmanageNG server would typically create one instance of the error reporter upon startup,
    // so that all error reports generated by the same running instance of a drbdmanageNG server
    // create error reports with the same instance ID.
    // When the drbdmanageNG server is restarted, the instance ID will change.
